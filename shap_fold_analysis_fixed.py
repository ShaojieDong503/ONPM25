#!/usr/bin/env python3
"""
shap_fold_analysis_fixed.py -- the corrected SHAP figure script.

PROVENANCE. The reviewer asked for a file of this name to be placed in the project
folder. No such file was received, so this is NOT the reviewer's script: it is the
former `shap_fold_analysis.py` with the reported indexing defect fixed. The unfixed
original has been DELETED rather than left beside this one -- two near-identical
scripts would only invite someone to run the wrong one. If the reviewer's own version
arrives it should replace this file, and the two should be diffed rather than assumed
equivalent.

THE DEFECT. `sv` (SHAP values) and `X` (inputs) carry all 651 columns in CANONICAL
order -- the order in `bundle["feature_cols"]`. The old code selected plot columns by
each feature's position in the IMPORTANCE-SORTED table:

    feats = list(tax.sort_values("rank")["feature"])     # importance order
    order = [feats.index(f) for f in tax.head(20)["feature"]]
    ...  summary_plot(sv[:, order], X[:, order], feature_names=...)

so rank k's data was drawn under rank k's name. Measured on the published fold-4
figures, the five most prominent panels were wrong:

    label printed                        data actually plotted
    src_merra_aer_BCSMASS_wmean_1000km   dayofyear
    src_merra_aer_BCSMASS_wmean_500km    air_sfc
    src_merra_aer_BCSMASS_wmean_250km    hpbl
    src_merra_aer_BCSMASS_point          windspeed_10m
    src_merra_slv_V2M_wmean_1000km       vis

THE FIX. Columns are taken from the canonical list, and each selected column's
mean|SHAP| is recomputed and asserted equal to the table value for that name, so a
recurrence fails loudly instead of plotting quietly.

SCOPE. Only the beeswarm (fig 4) and dependence (fig 5) panels were affected. The bar
chart, the repaired-share and family figures, and shap_feature_importance.csv are
computed per canonical column index and were always correct -- no reported NUMBER was
wrong for this reason.

  python shap_fold_analysis_fixed.py --fold 1 --stage all
  python shap_fold_analysis_fixed.py --fold 1 --stage figures     # no recompute
...
def compute_shap(fold: int, shard_root: Path, out_root: Path, limit: int | None) -> Path:
    ...
    print(f"[shap] fold {fold}: {len(features)} features, {len(heldout)} held-out blocks", flush=True)
    print(f"[shap] blocks: {', '.join(heldout)}", flush=True)

    meta, X, _ = F.load_blocks(shard_root, heldout, features, lg)
    if limit:
        meta, X = meta.iloc[:limit].copy(), X[:limit]
    print(f"[shap] explaining {X.shape[0]:,} rows x {X.shape[1]} features", flush=True)

    t0 = time.perf_counter()
    contrib = model.predict(X, pred_contrib=True)
    print(f"[shap] TreeSHAP done in {time.perf_counter() - t0:.0f}s", flush=True)
    ...
    print(f"[shap] additivity check: max |base + sum(shap) - pred| = {max_err:.3e}", flush=True)
    if max_err > 1e-3:
    ...
    print(f"[shap] wrote {sdir}", flush=True)
    return sdir
...
def make_figures(fold: int, out_root: Path, top_n: int = 25) -> None:
    ...
    shard_root = Path(summary.get("shard_root", ROOT / "Data"))
    _, X, _ = F.load_blocks(shard_root, bundle["holdout_blocks"],
                            list(bundle["feature_cols"]), lg)
    X = X[: sv.shape[0]]

    # `sv` and `X` carry all 651 columns in CANONICAL order. `tax` is sorted by
    # importance. Selecting columns by position in the importance-sorted list plots
    # rank k's data under rank k's name, which is only the same thing by coincidence
    # -- it previously put `dayofyear` under the top feature's label. Index into the
    # canonical order, and prove it by recomputing the statistic from the column
    # actually taken and matching it to the table row for that name.
    canonical = list(bundle["feature_cols"])
    top = list(tax.head(20)["feature"])
    order = [canonical.index(f) for f in top]
    # Tolerance is set from measurement, not taste. `shap_values.npy` is saved as
    # float32 while the table's mean_abs_shap was computed in float64 before the
    # cast, so an HONEST match still differs -- measured 1.9e-6 relative on fold 4.
    # A genuine column mix-up is far coarser: the smallest relative gap between two
    # distinct top-20 features on that fold is 2.7e-3, about 1400x larger. 1e-4 sits
    # between them, tolerating the round-trip with ~50x margin while still catching
    # a swap with ~27x margin. Tightening this to 1e-6 does not make the check
    # stronger, only flaky -- it fails on float32 noise and says "mislabelled".
    for k, f in zip(order, top):
        got = float(np.abs(sv[:, k]).mean())
        want = float(tax.loc[tax["feature"] == f, "mean_abs_shap"].iloc[0])
        if not np.isclose(got, want, rtol=1e-4, atol=1e-9):
            raise AssertionError(
                f"column {k} ({canonical[k]!r}) does not carry {f!r}: "
                f"mean|SHAP| {got!r} vs table {want!r} "
                f"(relative {abs(got - want) / max(abs(want), 1e-30):.3e})")
    ...
    for ax, feat in zip(np.atleast_1d(axes), tax.head(n_dep)["feature"]):
        j = canonical.index(feat)          # canonical order, as for fig 4
    ...
    print(f"[shap] figures written to {sdir}", flush=True)
...
def main() -> int:
    ...
    ap.add_argument("--out-root", type=Path,
                    default=ROOT / "outputs" / "lgbm_thesis")
